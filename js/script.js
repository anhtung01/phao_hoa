...
const helpContent = {
  shellType: {
    header: 'Loại pháo',
    body: 'Bạn có thể chọn loại pháo mà bạn muốn bắn, chọn "Ngẫu nhiên" để có trải nghiệm tuyệt vời!',
  },
  shellSize: {
    header: 'Kích thước pháo',
    body: 'Pháo càng lớn thì phạm vi nở càng lớn, nhưng pháo lớn hơn sẽ yêu cầu hiệu suất thiết bị cao hơn, pháo lớn có thể khiến thiết bị của bạn bị lag.',
  },
  quality: {
    header: 'Chất lượng hình ảnh',
    body: 'Nếu hoạt ảnh không chạy mượt mà, bạn có thể thử giảm chất lượng hình ảnh. Chất lượng hình ảnh càng cao, số lượng tia lửa sau khi pháo nở càng nhiều, nhưng chất lượng cao có thể khiến thiết bị của bạn bị lag.',
  },
  skyLighting: {
    header: 'Chiếu sáng bầu trời',
    body: 'Khi pháo nổ, nền sẽ được chiếu sáng. Nếu màn hình của bạn trông quá sáng, bạn có thể chuyển sang "Tối" hoặc "Không".',
  },
  scaleFactor: {
    header: 'Tỷ lệ',
    body: 'Giúp bạn gần hơn hoặc xa hơn với pháo. Đối với pháo lớn hơn, bạn có thể chọn giá trị tỷ lệ nhỏ hơn, đặc biệt là trên điện thoại hoặc máy tính bảng.',
  },
  autoLaunch: {
    header: 'Tự động bắn pháo',
    body: 'Khi bật, bạn có thể ngồi trước màn hình thiết bị của mình để thưởng thức pháo, bạn cũng có thể tắt nó, nhưng khi tắt, bạn chỉ có thể bắn pháo bằng cách nhấp vào màn hình.',
  },
  finaleMode: {
    header: 'Bắn nhiều pháo cùng lúc',
    body: 'Có thể tự động bắn nhiều pháo hơn cùng một lúc (nhưng cần bật "Tự động bắn pháo" trước).',
  },
  hideControls: {
    header: 'Ẩn nút điều khiển',
    body: 'Ẩn các nút ở đầu màn hình. Nếu bạn muốn chụp màn hình hoặc cần trải nghiệm liền mạch, bạn có thể ẩn các nút, sau khi ẩn nút, bạn vẫn có thể mở cài đặt ở góc trên bên phải.',
  },
  fullscreen: {
  ...
  longExposure: {
    header: 'Giữ lại tia lửa của pháo',
    body: 'Có thể giữ lại tia lửa của pháo',
  },
};
...
let happyNewYearState = {
  isVisible: false, // Trạng thái hiển thị chữ
  lastShowTime: 0, // Thời gian hiển thị
  fadeDuration: 3000, // Thời gian mờ dần chữ (3 giây)
  colors: ['#FF0000', '#00FF00', '#0000FF', '#FFFF00', '#FF00FF', '#00FFFF'], // Màu sắc động
  currentColor: 0, // Màu sắc hiện tại
};

function shouldShowHappyNewYear() {
  const currentTime = Date.now();

  // Kiểm tra xem có vụ nổ pháo hoa vừa xảy ra không
  if (BurstFlash.active.length > 0) {
    // Hiển thị chữ ngay khi có pháo hoa nổ
    if (!happyNewYearState.isVisible) {
      happyNewYearState.isVisible = true;
      happyNewYearState.lastShowTime = currentTime; // Ghi lại thời gian hiển thị
      happyNewYearState.currentColor =
        (happyNewYearState.currentColor + 1) % happyNewYearState.colors.length; // Đổi màu
    }
  }

  // Nếu chữ đang hiển thị, mờ dần theo thời gian
  if (happyNewYearState.isVisible) {
    const fadeProgress =
      (currentTime - happyNewYearState.lastShowTime) /
      happyNewYearState.fadeDuration;

    // Nếu thời gian đã qua lớn hơn fadeDuration, ẩn chữ
    if (fadeProgress >= 1) {
      happyNewYearState.isVisible = false;
    }
  }

  return happyNewYearState.isVisible;
}

function renderHappyNewYear(ctx, canvasWidth, canvasHeight) {
  const state = happyNewYearState;

  // Nếu không cần hiển thị, không làm gì
  if (!state.isVisible) return;

  // Danh sách font chữ cho phép bạn chọn
  const fonts = [
    'bold 72px Arial', // Font mặc định
    "bold 72px 'Comic Sans MS'", // Comic Sans MS
    "bold 72px 'Courier New'", // Courier New
    "bold 72px 'Georgia'", // Georgia
    "bold 72px 'Times New Roman'", // Times New Roman
    "bold 72px 'Verdana'", // Verdana
    "italic 72px 'Lucida Console'", // Lucida Console
  ];

  const selectedFont = fonts[0]; // Lựa chọn font đầu tiên

  // Cấu hình font chữ và màu sắc
  ctx.save();
  ctx.font = selectedFont; // Font chữ chọn được
  ctx.fillStyle = state.colors[state.currentColor]; // Màu sắc động
  ctx.textAlign = 'center'; // Căn giữa chữ
  ctx.textBaseline = 'middle'; // Căn giữa theo chiều dọc

  // Tính toán độ mờ dần của chữ
  const fadeProgress = (Date.now() - state.lastShowTime) / state.fadeDuration;
  const alpha = 1 - fadeProgress; // Độ mờ dần (từ 1 đến 0)

  ctx.globalAlpha = Math.max(0, alpha); // Đảm bảo độ mờ dần không nhỏ hơn 0
...
function render(speed) {
  ...
  // Render Happy New Year message (call the function when conditions are met)
  if (shouldShowHappyNewYear()) {
    renderHappyNewYear(mainCtx, width, height); // Vẽ chữ lên canvas
  }
